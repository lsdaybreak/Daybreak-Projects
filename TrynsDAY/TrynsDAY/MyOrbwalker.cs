...
{   
        ...
        public static class MyOrbwalker
        {
                ...
                if (_championName == "Rengar")
                {
                    Obj_AI_Base.OnPlayAnimation += delegate (Obj_AI_Base sender, GameObjectPlayAnimationEventArgs args)
                    {
                        if (sender.IsMe && args.Animation == "Spell5")
                        {
                            ...
                            LastAATick = LeagueSharp.Common.Utils.GameTimeTickCount - Game.Ping / 2 + t;
                        }
                    };
                }
            }
            ...
            public static bool CanAttack()
            {
                ...
                return LeagueSharp.Common.Utils.GameTimeTickCount + Game.Ping / 2 + 25 >= LastAATick + Player.AttackDelay * 1000;
            }
                ...
                bool randomizeMinDistance = true)
            {
                ...
                if (LeagueSharp.Common.Utils.GameTimeTickCount - LastMoveCommandT < 70 + Math.Min(60, Game.Ping) && !overrideTimer
                    && angle < 60)
                {
                    return;
                }
                ...
                public Orbwalker(Menu attachToMenu)
                {
                    ...
                    drawings.AddItem(
                        new MenuItem("HoldZone", "HoldZone").SetShared()
                            .SetValue(new Circle(false, Color.LightSeaGreen)));
                ...
                public virtual AttackableUnit GetTarget()
                {
                        ...
                        || mode == OrbwalkingMode.Freeze)
                    {
                        ...
                        foreach (var minion in MinionList)
                        {
                            ...
                            var predHealth = LeagueSharp.Common.HealthPrediction.GetHealthPrediction(minion, t, this.FarmDelay);
                            ...
                            if (minions.Any())
                            {
                                ...
                                var turretMinion =
                                    minions.FirstOrDefault(
                                        minion =>
                                        minion is Obj_AI_Minion && LeagueSharp.Common.HealthPrediction.HasTurretAggro(minion as Obj_AI_Minion));
                                ...
                                if (turretMinion != null)
                                {
                                    ...
                                    var turretStarTick = LeagueSharp.Common.HealthPrediction.TurretAggroStartTick(
                                        turretMinion as Obj_AI_Minion);
                                    // from LeagueSharp.Common.HealthPrediction (don't blame me :S)
                                         ...
                                         i = i + closestTower.AttackDelay * 1000)
                                    {
                                        ...
                                        var predHP =
                                            (int)
                                            LeagueSharp.Common.HealthPrediction.LaneClearHealthPrediction(turretMinion, time > 0 ? time : 0);
                                        if (predHP > 0)
                                        {
                                            hpLeft = predHP;
                                            turretAttackCount += 1;
                                    ...
                                    foreach (var minion in
                                        minions.Where(
                                            ...
                                            && !LeagueSharp.Common.HealthPrediction.HasMinionAggro(x as Obj_AI_Minion)))
                                    {
                                ...
                                else
                                {
                                    ...
                                    foreach (var minion in
                                        minions.Where(
                                            x => x is Obj_AI_Minion && !LeagueSharp.Common.HealthPrediction.HasMinionAggro(x as Obj_AI_Minion))
                                        )
                                    {
                    ...
                    if (mode == OrbwalkingMode.LaneClear)
                    {
                        if (!this.ShouldWait())
                        {
                            ...
                            {
                                var predHealth = LeagueSharp.Common.HealthPrediction.LaneClearHealthPrediction(
                                    this._prevMinion,
